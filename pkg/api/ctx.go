...
func (c *Ctx) reset(w http.ResponseWriter, r *http.Request) {
	traceId := r.Header.Get(opt.TraceKey)
	if traceId == "" {
	...
	c.Request = r.WithContext(context.WithValue(r.Context(), opt.TraceKey, traceId))
	...
	*c.skippedNodes = (*c.skippedNodes)[:0]
	...
	c.writermem.Header().Set(opt.TraceKey, traceId)
}
